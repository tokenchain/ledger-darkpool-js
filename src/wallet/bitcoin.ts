import {Transaction, script, address} from "bitcoinjs-lib"
import request from "request"
import bs58check from "bs58check"
import secp256k1 from "secp256k1"
import {sha2, ripemd160} from "./hash"
import {BASE_URL, byte, concat} from "./util"
...
function createFinalTx(inputs, feeRate) {
    ...
    for (let output of inputs) {
        ...
        let txid = new Buffer(txhashhex, 'hex')
    ...
    let cosmosAddressScript = script.nullDataOutput(new Buffer(20).fill(0))
...
function signFinalTx(wallet, tx) {
    ...
    let cosmosAddress = new Buffer(wallet.addresses.cosmos.slice(2), 'hex')
